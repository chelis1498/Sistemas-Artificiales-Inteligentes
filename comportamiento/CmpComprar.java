 ...
 * @author OGChelis
 */
public class CmpComprar extends Behaviour{
    private AID mejorVendedor;
    private int mejorPrecio;
    private int repliesCount = 0;
    private MessageTemplate mt;
    private int step = 0;
    private AGcomprador bbAgent;
    private String tituloLibro;

    public CmpComprar(AGcomprador a) {
        bbAgent = a;
            ...
            case 3:      
                ...
                if (reply != null) {
                    if (reply.getPerformative() == ACLMessage.INFORM) {
                        JOptionPane.showMessageDialog(bbAgent.intfz,tituloLibro+" - $"+mejorPrecio+" comprado a: "+reply.getSender().getName(),"Lbro comprado",JOptionPane.INFORMATION_MESSAGE);
                        myAgent.doDelete();
                    }
                    else {
                        JOptionPane.showMessageDialog(bbAgent.intfz, "El libro ya no esta disponible","Error",JOptionPane.INFORMATION_MESSAGE);
                    }
                    step = 4;
                }
                else {
    ...
    public boolean done() {
        if (step == 2 && mejorVendedor == null) {
            JOptionPane.showMessageDialog(bbAgent.intfz,tituloLibro+" no esta disponible","Intento fallido",JOptionPane.INFORMATION_MESSAGE);
        }
